...
impl<'a> Iterator for GetEntriesIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.next_index == last_gotten_range.end {
            ...
            let mut next_sub_range = last_gotten_range.end
                ..u64::min(
                    last_gotten_range.end + self.batch_size,
                    self.requested_range.end,
                );
            let try_next_entries = get_json(
                self.client,
                self.base_url,
                &format!(
                    "ct/v1/get-entries?start={}&end={}",
                    next_sub_range.start,
                    next_sub_range.end - 1
                ),
            )
            .map(|x: jsons::GetEntries| x.entries);
            if let Ok(next_entries) = try_next_entries {
                ...
                if next_entries.is_empty() {
                    ...
                    self.last_gotten_entries =
                        (next_sub_range, next_entries.into_iter().map(Some).collect());
                    self.next_index += 1;
                    let leaf_entry = self.last_gotten_entries.1[0].take().unwrap();
                    match Leaf::try_from(&leaf_entry) {
                        Ok(leaf) => Some(Ok(leaf)),
                        Err(e) => {
                            self.done = true;
                            Some(Err(e))
                        }
            ...
            assert_eq!(
                last_gotten_entries.len() as u64,
                last_gotten_range.end - last_gotten_range.start
            );
            let leaf_entry = last_gotten_entries
                [(self.next_index - last_gotten_range.start) as usize]
                .take()
                .unwrap();
            self.next_index += 1;
            match Leaf::try_from(&leaf_entry) {
